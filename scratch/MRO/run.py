...
for i in range(1):
    ...
    while not r1.isFinish():
        with r1 as data:
            if data == None:
                break
            relativeDistanceX = abs(data.env.x - rfConfig["BS"][floor(data.env.cellId/3) + (data.env.cellId % 3)]["location"][0])
            print(relativeDistanceX)
            relativeDistanceY = abs(data.env.y - rfConfig["BS"][floor(data.env.cellId/3) + (data.env.cellId % 3)]["location"][1])
